...
#ifdef DEBUG
void microcoap_dumpHeader(microcoap_header_t *hdr)
{
    printf("Header:\n");
    printf("  ver  0x%02X\n", hdr->ver);
    printf("  t    0x%02X\n", hdr->t);
...
int microcoap_parseHeader(microcoap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
    if (buflen < 4)
...
int microcoap_parseToken(microcoap_buffer_t *tokbuf, const microcoap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
    if (hdr->tkl == 0)
    {
        tokbuf->p = NULL;
...
// advances p
int microcoap_parseOption(microcoap_option_t *option, uint16_t *running_delta, const uint8_t **buf, size_t buflen)
{
    const uint8_t *p = *buf;
    uint8_t headlen = 1;
    uint16_t len, delta;
...
int microcoap_parseOptionsAndPayload(microcoap_option_t *options, uint8_t *numOptions, microcoap_buffer_t *payload, const microcoap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
    ...
    {
        if (0 != (rc = microcoap_parseOption(&options[optionIndex], &delta, &p, end-p)))
            return rc;
        optionIndex++;
    }
...
#ifdef DEBUG
void microcoap_dumpOptions(microcoap_option_t *opts, size_t numopt)
{
    size_t i;
    printf(" Options:\n");
    for (i=0;i<numopt;i++)
    {
        ...
        microcoap_dump(opts[i].buf.p, opts[i].buf.len, true);
        printf(" ]\n");
    }
}
#endif
...
{
	microcoap_dumpHeader(&pkt->hdr);
	microcoap_dumpOptions(pkt->opts, pkt->numopts);
    printf("Payload: ");
    microcoap_dump(pkt->payload.p, pkt->payload.len, true);
    printf("\n");
}
#endif
...
int microcoap_parse(microcoap_packet_t *pkt, const uint8_t *buf, size_t buflen)
{
    ...
    // microcoap_dump(buf, buflen, false);

    if (0 != (rc = microcoap_parseHeader(&pkt->hdr, buf, buflen)))
        return rc;
//    microcoap_dumpHeader(&hdr);
    if (0 != (rc = microcoap_parseToken(&pkt->tok, &pkt->hdr, buf, buflen)))
        return rc;
    pkt->numopts = MAXOPT;
    if (0 != (rc = microcoap_parseOptionsAndPayload(pkt->opts, &(pkt->numopts), &(pkt->payload), &pkt->hdr, buf, buflen)))
        return rc;
//    microcoap_dumpOptions(opts, numopt);
    return 0;
}
...
const microcoap_option_t *microcoap_findOptions(const microcoap_packet_t *pkt, uint8_t num, uint8_t *count)
{
    ...
    size_t i;
    const microcoap_option_t *first = NULL;
    *count = 0;
...
int microcoap_build(uint8_t *buf, size_t *buflen, const microcoap_packet_t *pkt)
{
    ...
    for (i=0;i<pkt->numopts;i++)
    {
        ...
        microcoap_option_nibble(optDelta, &delta);
        microcoap_option_nibble((uint32_t)pkt->opts[i].buf.len, &len);
...
int microcoap_handle_req(microcoap_rw_buffer_t *scratch, const microcoap_packet_t *inpkt, microcoap_packet_t *outpkt)
{
    ...
    while(NULL != ep->handler)
    {
        if (ep->method != inpkt->hdr.code)
            goto next;
        if (NULL != (opt = microcoap_findOptions(inpkt, COAP_OPTION_URI_PATH, &count)))
        {
...
next:
    ...
    microcoap_make_response(scratch, outpkt, NULL, 0, inpkt->hdr.id[0], inpkt->hdr.id[1], &inpkt->tok, COAP_RSPCODE_NOT_FOUND, COAP_CONTENTTYPE_NONE);
